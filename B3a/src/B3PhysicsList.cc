...
#include "B3PhysicsListMessenger.cc"
...
B3PhysicsList::B3PhysicsList() : G4VModularPhysicsList()
{
  ...
  fMessenger = new B3PhysicsListMessenger(this);
...
void B3PhysicsList::AddPhysicsList(const G4String& name)
{
  if (verboseLevel>0) {
    G4cout << "B3PhysicsList::AddPhysicsList: <" << name << ">" << G4endl;
  }
  if (name == "emstandard_opt0") {
    ...
    G4cout << "B3PhysicsList::AddPhysicsList: <" << name << ">"
           << " is not defined"
           << G4endl;
  }
}
...
void B3PhysicsList::SetBuilderList0(G4bool flagHP)
{
...
void B3PhysicsList::SetBuilderList1(G4bool flagHP)
{
  fHadronPhys.push_back( new G4EmExtraPhysics(verboseLevel));
  if(flagHP) {
    fHadronPhys.push_back( new G4HadronElasticPhysicsHP(verboseLevel) );
...
void B3PhysicsList::SetBuilderList2()
{
  fHadronPhys.push_back( new G4EmExtraPhysics(verboseLevel));
